...
log_debug("core.main_app module initialized.")

from app.processing.model_loader import (
    load_model as initial_load_model,
    get_available_model_keys,
    get_default_model_key,
)
from app.ui.elements import create_ui_components
from app.ui.handlers.loading_manager import show_loading, hide_loading_and_update_controls
from app.ui.callbacks import init_callbacks
...
def on_close(root_win, from_exception=False):
    ...
    from app.ui.callbacks import _stop_all_processing_logic
...
def launch_app():
    ...
    log_debug("core.main_app: launch_app() called.")
...
if __name__ == "__main__":
    # This block is for direct execution, ensure config is usable
    # Minimal setup for direct run, assuming app parent is in path
    try:
        from app import config as main_app_config
    except ImportError:
        # Fallback if app structure isn't in path (e.g. running core/main_app.py directly from core/)
        # This might lead to issues if other app.x imports are needed without further sys.path manipulation
        import os
        import sys
        sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..')))
        from app import config as main_app_config
        print("main_app.py: Had to adjust sys.path for direct execution.")

    print("main_app.py executed directly. For proper execution, use run_app.py.")
    if not main_app_config.IS_DEBUG_MODE:
        print("Debug mode not enabled via run_app.py. Basic logging might be limited.")
    # For direct execution, logging might not be fully set up by run_app.py
    # Consider a basic setup here if critical, or rely on prints for this specific case.
    # from app.utils.logger_setup import setup_logging
    # setup_logging() # This would use config.IS_DEBUG_MODE from the imported config
    launch_app()
